...
def main(args):
    ...
    if args.train:
        ...
        for fold_id in range(4):
            ...
            if args.use_wandb: 
                ...
                wandb.init(
                    project=save_dir.parents[2].name+"_"+save_dir.parents[1].name+"_GroupConstraintMIL_"+save_dir.name,
    ...
    if args.eval_static:
        ...
        new_method_base_root = save_groupConstraint_base_dir if args.train else Path(args.test_dir) if args.test_dir is not None and args.test_dir != "" else Path(os.path.join(args.save_dir, "groupConstraint"))
        baseline_base_root = Path(args.baseline_dir) if args.baseline_dir is not None and args.baseline_dir != "" else Path(os.path.join(args.save_dir, "baseline"))

        # check probability csv exist
        for fold_id in range(4):

            csv_file_path = csv_file_path.with_name(f"dataset_fold_{fold_id}" + csv_file_path.suffix)
                ...
                if not os.path.isfile(os.path.join(baseline_root, "baseline.pth")):
                    ...
                    if args.use_wandb: 
                        ...
                        wandb.init(
                            ...
                            name=run_name+ "_" + Runtime + args.log_name,     
                            config=vars(args)                    
                        )
                    else:
                        print("No wandb")
